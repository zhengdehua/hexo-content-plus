...
const { Util } = require('hexo-content-plus/lib/util');
const util = new Util();
...
const initTypes = function (prefixedKey) {
  // 定义基础消息类型及其对应的颜色和图标
  let types = {
    info: "#00b8d4 || fa fa-circle-info",    // 信息类型：青色 + 信息图标
    success: "#00c853 || fa fa-check",       // 成功类型：绿色 + 对勾图标
    warning: "#ff9100 || fa fa-triangle-exclamation", // 警告类型：橙色 + 警告图标
    failure: "#ff5252 || fa fa-xmark",       // 失败类型：红色 + 叉号图标
    quote: "#9e9e9e || fa fa-quote-right",   // 引用类型：灰色 + 引用图标
  };
...
class AdmonitionRenderer {
  ...
  render(args, content, post) {
    ...
    const block = util.renderContent(content, post, this.hexo);
...
<details class="${this.nh}-content block ${fullType}"${open ? ' open' : ''}>
  ...
  contentCards(args, content, post) {
    ...
    for (let i = 0; i < contents.length; i++) {
      contents[i] = util.renderContent(contents[i], post, this.hexo);
    }
